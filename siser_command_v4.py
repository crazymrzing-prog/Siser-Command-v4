...
#
# Siser Command v4 (single-tool)
#
# This is the Inkscape entry point (the script an .inx file's
# <command> points to). All heavy lifting lives in the sibling
# siser_*.py modules in this same directory.
#
# Ported from SkyCut Command Model C3 (skycut_command_model_c3.py) -
# same modular architecture, same tool model. The only functional
# change from SkyCut is in siser_hpgl.py: Siser's coordinate
# convention (bottom-left origin, 90-degree rotation) instead of
# SkyCut's swap+invert. Everything else below is unchanged:
#
#   siser_constants.py  - shared master switches / numeric constants
#   siser_geometry.py   - pure vector math + Bezier flattening
#   siser_arrows.py     - debug direction-arrow polylines
#   siser_pathbuild.py  - knife-offset corner overshoot/return/overcut
#   siser_reference.py  - wysiwyg/origin/contourcut reference-box logic
#   siser_layers.py     - SVG layer management + geometry collection (mixin)
#   siser_hpgl.py        - HPGL command assembly
#   siser_output.py      - save-to-file / send-to-cutter delivery
#   siser_usb.py         - Windows USB-Printer device discovery + raw send,
#                              used by siser_output.py for Send to Machine USB
#   siser_viewer.py      - Preview Path HTML viewer
...
from siser_layers import SiserLayerMixin
from siser_pathbuild import build_cut_path
from siser_reference import compute_reference_box, get_contour_mark_paths, ReferenceError
from siser_hpgl import assemble_hpgl, build_render_segments
from siser_output import save_hpgl_file, send_hpgl, send_hpgl_usb
from siser_viewer import open_hpgl_viewer
from siser_layer_setup import run_layer_setup
from siser_add_marks import run_add_marks
from siser_constants import SOURCE_LAYER_NAMES, LAYER_DISPLAY_NAMES

# Actual SVG layer names to search the document for, per kind - see
# SOURCE_LAYER_NAMES in siser_constants.py (also used by
# siser_layer_setup.py, so the two stay in sync within this
# extension).
LAYER_LABELS = SOURCE_LAYER_NAMES


class SiserCommand_V4(SiserLayerMixin, inkex.EffectExtension):
    ...
    def effect(self):
        ...
        if not any_geometry:
            inkex.errormsg(
                "No geometry found in the layer(s) assigned to Tool; aborting."
            )
            return
        ...
        # siser_reference.py for the coordinate-system explanation.
        try:
        ...
        # natural (unrotated) Inkscape orientation - Siser's axis
        ...
        if output_mode in ("preview_path", "preview_web"):
            ...
            # order). All three modes use bottom-left (min_x, max_y) -
            # this matches the real HPGL formula
            # (hpgl_x = max_y - y, hpgl_y = x - min_x), which sends
            # a reference box's bottom-left corner to machine (0,0),
            # regardless of mode. (Siser's formula is fully
            # box-relative here, the same way SkyCut's is, just
            # mirrored to the other corner - see siser_hpgl.py.)
            origin_point = (min_x, max_y)
