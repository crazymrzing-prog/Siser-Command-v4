...
"""
siser_layer_setup.py

Creates and updates the standard Siser cutter layers so a new
document starts with the right layer names/colors already set up -
the exact names siser_command_model_c.py's collect_layer() searches
for (see SOURCE_LAYER_NAMES in siser_constants.py), plus Print and
...
siser_command_model_c.py's effect() when Output is set to "Layer
...
from siser_constants import PREVIEW_COLORS, SOURCE_LAYER_NAMES
...
# siser_command_model_c.py calls run_layer_setup() directly instead)
# ==========================================================
