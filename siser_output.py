...
from siser_usb import find_usbprint_paths, send_job, friendly_label, pick_and_send
...
def save_hpgl_file(data, cut_mode):
    ...
    filename = f"Siser_{cut_mode}_{datetime.now():%Y%m%d%H%M%S}.plt"
...
def send_hpgl(data, ip, port, connect_timeout=5, send_timeout=90):
    ...
    try:
        with socket.create_connection((ip, port), timeout=connect_timeout) as s:
            ...
            s.sendall((data + "\n").encode("utf-8"))
        return True, "Sent to Siser successfully"
...
def send_hpgl_usb(data, device_path=""):
    ...
    (e.g. "siser_5503") or the full raw device interface path - if
    ...
    confirmation window (pick_and_send(), in siser_usb.py) so
    ...
    if entered:
        ...
        # (e.g. "siser_5503") rather than the full raw device
    ...
    if not paths:
        return False, (
            "No Siser device found (USB ID 0483:5750, usbprint.sys). "
    ...
    try:
        ...
        return False, (
            ...
            "than one Siser device was found - enter one of these labels into the "
