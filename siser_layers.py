...
Implemented as a mixin (SiserLayerMixin) so the main extension class
...
from siser_geometry import flatten_superpath_to_subpaths, dedupe, is_cw
from siser_arrows import direction_arrows
from siser_constants import CAPTURE_ENABLE_MASTER, CAPTURE_STROKE_WIDTH, FLATTEN_TOLERANCE_MM, PREVIEW_COLORS, LAYER_DISPLAY_NAMES


class SiserLayerMixin:
    """Layer/geometry helpers shared by the Siser effect extension."""
    ...
    # elements, same blind spot - to_path_element() does not do real
    # glyph outlining for text, so text is routed through
    # convert_text_to_paths() instead - see convert_shapes_to_paths()
    # below.
